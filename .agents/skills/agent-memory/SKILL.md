...
**Location:** `.opencode/skills/agent-memory/memories/`
...
```bash
# 1. List categories
ls .opencode/skills/agent-memory/memories/

# 2. View all summaries
rg "^summary:" .opencode/skills/agent-memory/memories/ --no-ignore --hidden

# 3. Search summaries for keyword
rg "^summary:.*keyword" .opencode/skills/agent-memory/memories/ --no-ignore --hidden -i

# 4. Search by tag
rg "^tags:.*keyword" .opencode/skills/agent-memory/memories/ --no-ignore --hidden -i

# 5. Full-text search (when summary search isn't enough)
rg "keyword" .opencode/skills/agent-memory/memories/ --no-ignore --hidden -i
...
```bash
mkdir -p .opencode/skills/agent-memory/memories/category-name/
# Note: Check if file exists before writing to avoid accidental overwrites
cat > .opencode/skills/agent-memory/memories/category-name/filename.md << 'EOF'
---
...
- **Delete**: Remove memories that are no longer relevant
  ```bash
  trash .opencode/skills/agent-memory/memories/category-name/filename.md
  # Remove empty category folders
  rmdir .opencode/skills/agent-memory/memories/category-name/ 2>/dev/null || true
  ```
