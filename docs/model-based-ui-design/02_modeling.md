...
本システムにおける主要なデータは、事前に定義された「鑑定パターン」という静的な参照データです。
...
classDiagram
    ...
    AppraisalPattern "1" *-- "1" EnergyUpColors : contains
    ApplicationState "0..1" --> "1" AppraisalPattern : selects
```

### オブジェクト詳細

#### AppraisalPattern (鑑定パターン)
...
stateDiagram-v2
    ...
    Selecting --> Previewing : 鑑定パターンを選択して決定
...
* **Selecting (選択状態)**: 一覧から該当する鑑定パターンを探している状態。
* **Previewing (確認状態)**: 選択した鑑定パターンの鑑定書イメージを確認している状態。
...
```mermaid
flowchart LR
    Start(開始) --> SelectScreen[入力画面: 鑑定パターンを選ぶ]
...
1. **鑑定パターン選択**: 占い師は、手元の判定結果（例：「子・丑のマイナス」）と画面上のリストを照合し、該当する鑑定パターンを選択する。
